        ...
        do j = 1, ny-1
            do i = 1, nx-1
                do m = 0, num_materials
                    print*, 'aaaaa', j, i, m, pressure_sum%get_item(m, i, j),pressure_sum%get_item(m, i - 1, j),&
                     pressure_sum%get_item(m, i + 1, j) ,pressure_sum%get_item(m, i, j - 1),  pressure_sum%get_item(m, i, j + 1)
                        temp = 0.25d0 * (pressure_sum%get_item(m, i, j)               &
                                        + pressure_sum%get_item(m, i - 1, j) &
                                        + pressure_sum%get_item(m, i + 1, j) &
                                        + pressure_sum%get_item(m, i, j - 1) &
                                        + pressure_sum%get_item(m, i, j + 1))
                end do
            end do
        end do
