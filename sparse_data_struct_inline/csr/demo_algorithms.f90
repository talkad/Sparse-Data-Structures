            ...
            do j = 0, ny
                do i = 0, nx
                    do m = 1, num_materials
                            ...
                            a_val = 0
                            idx = A%idx_map(m, i, j, k)
                            if (idx > -1) a_val = A%values(idx)

                            b_val = 0
                            idx = B%idx_map(m, i, j, k)
                            if (idx > -1) b_val = B%values(idx)
            ...
            do j = 1, ny-1
                do i = 1, nx-1
                    do m = 1, num_materials
                        ...
                        curr = 0
                        idx = A%idx_map(m, i, j, k)
                        if (idx > -1) curr = A%values(idx)

                        up = 0
                        idx = A%idx_map(m, i-1, j, k)
                        if (idx > -1) up = A%values(idx)

                        down = 0
                        idx = A%idx_map(m, i+1, j, k)
                        if (idx > -1) down = A%values(idx)

                        left = 0
                        idx = A%idx_map(m, i, j-1, k)
                        if (idx > -1) left = A%values(idx)

                        right = 0
                        idx = A%idx_map(m, i, j+1, k)
                        if (idx > -1) right = A%values(idx)

                        bottom = 0
                        idx = A%idx_map(m, i, j, k-1)
                        if (idx > -1) bottom = A%values(idx)

                        top = 0
                        idx = A%idx_map(m, i, j, k+1)
                        if (idx > -1) top = A%values(idx)
