module mat_array_module
    ...
    function get_item(this, material_type, i, j)
        ...
        ! print*, 'a', this%mat(i, j)%types(this%curr), material_type
    ...
    subroutine get_neighborhood(this, material_type, i, j, curr_val, upper_val, lower_val, left_val, right_val)
        ...
        if (this%mat(i, j)%types(this%up) == material_type) then
            upper_val = this%mat(i, j)%values(this%up)
            this%up = this%up + 1
        end if

        if (this%mat(i, j)%types(this%down) == material_type) then
            lower_val = this%mat(i, j)%values(this%down)
            this%down = this%down + 1
        end if

        if (this%mat(i, j)%types(this%left) == material_type) then
            left_val = this%mat(i, j)%values(this%left)
            this%left = this%left + 1
        end if

        if (this%mat(i, j)%types(this%right) == material_type) then
            right_val = this%mat(i, j)%values(this%right)
