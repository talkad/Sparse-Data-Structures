module algorithm_module
    ...
    subroutine intensive_algorithm_neighbors(pressure_sum, nx, ny, num_materials, idx_map)
        class(mat_array_t), pointer, intent(in) :: pressure_sum
        ...
        do j = 1, ny-1
            do i = 1, nx-1
                do m = 0, num_materials
                    ! print*, '-----------------'
                    ! print*, j,i,m
                    call pressure_sum%get_neighbors(m,i,j, curr, up, down, left, right, idx_map)
    ...
    subroutine intensive_algorithm(pressure_sum, nx, ny, num_materials, idx_map)
        class(mat_array_t), pointer, intent(in) :: pressure_sum
        integer, dimension(:,:,:), allocatable, intent(inout) :: idx_map
        integer, intent(in) :: nx,ny,num_materials
        ...
        do j = 0, ny
            do i = 0, nx
                do m = 0, num_materials
                        ...
                        temp_sum = temp_sum + 0.25d0 * pressure_sum%get_item(m, i, j, idx_map)
                end do
            end do
        end do
    ...
    subroutine intensive_algorithm_mat(pressure_sum, nx, ny, num_materials, idx_map)
        class(mat_array_t), pointer, intent(in) :: pressure_sum
        integer, dimension(:,:,:), allocatable, intent(inout) :: idx_map
        integer, intent(in) :: nx,ny,num_materials
