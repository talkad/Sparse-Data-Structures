program main
    ...
    ! print*, '----------'
    data_struct => mat_array_constructor(p, idx_map)
    ! print*, '----------'
    ...
    print*, nz_ratio
    time = omp_get_wtime()
    call intensive_algorithm(data_struct, nx, ny, num_materials, idx_map)
    print*, 'func time', omp_get_wtime() - time

    time = omp_get_wtime()
    call intensive_algorithm_mat(data_struct, nx, ny, num_materials, idx_map)
    print*, 'func time', omp_get_wtime() - time

    time = omp_get_wtime()
    call intensive_algorithm_neighbors(data_struct, nx, ny, num_materials, idx_map)
    print*, 'func time', omp_get_wtime() - time
